...
async def process_cart(message: Message, state: FSMContext):
    ...
    cart_data = await db.fetchall(
        'SELECT * FROM cart WHERE cid=$1', (message.chat.id,))
    ...
    if len(cart_data) == 0:
        ...
        for _, idx, count_in_cart in cart_data:
            ...
            product = await db.fetchone('SELECT * FROM products WHERE idx=$1', (idx,))

            if product == None:

                await db.query('DELETE FROM cart WHERE idx=$1', (idx,))
        ...
        async with state.proxy() as data:
            ...
            if 'products' not in data.keys():
                ...
                if count_in_cart == 0:
                    ...
                    WHERE cid = $1 AND idx = $2''', (query.message.chat.id, idx))
                    ...
                    SET quantity = $1 
                    WHERE cid = $2 AND idx = $3''', (count_in_cart, query.message.chat.id, idx))
    ...
    if enough_money:
        ...
        async with state.proxy() as data:
            ...
            products = [idx + '=' + str(quantity)
                        for idx, quantity in await db.fetchall('''SELECT idx, quantity FROM cart
            WHERE cid=$1''', (cid,))]  # idx=quantity

            await db.query('INSERT INTO orders VALUES ($1, $2, $3, $4)',
                           (cid, data['name'], data['address'], ' '.join(products)))

            await db.query('DELETE FROM cart WHERE cid=$1', (cid,))
