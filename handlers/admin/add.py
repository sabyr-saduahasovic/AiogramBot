...
async def category_callback_handler(query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    products = await db.fetchall('''SELECT * FROM products product
    WHERE product.tag = (SELECT title FROM categories WHERE idx=$1)''', (category_idx,))
...
async def set_category_title_handler(message: Message, state: FSMContext):
    ...
    await db.query('INSERT INTO categories(idx, title) VALUES ($1, $2)', (idx, category))
...
async def delete_category_handler(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if 'category_index' in data.keys():
            ...
            await db.query(
                'DELETE FROM products WHERE tag IN (SELECT title FROM categories WHERE idx=$1)', (idx,))
            await db.query('DELETE FROM categories WHERE idx=$1', (idx,))
...
async def process_confirm(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        cat = await db.fetchone(
            'SELECT title FROM categories WHERE idx=$1', (data['category_index'],))
        tag = cat[0]
        idx = md5(' '.join([title, body, price, tag]
                           ).encode('utf-8')).hexdigest()

        await db.query('INSERT INTO products(idx,title,body,photo,price,tag) VALUES ($1, $2, $3, $4, $5, $6)',
...
async def delete_product_callback_handler(query: CallbackQuery, callback_data: dict):
    ...
    await db.query('DELETE FROM products WHERE idx=$1', (product_idx,))
