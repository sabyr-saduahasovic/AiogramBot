...
async def process_questions(message: Message):
    ...
    questions = await db.fetchall('SELECT * FROM questions')
...
async def process_send_answer(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        question = await db.fetchone(
            'SELECT question FROM questions WHERE cid=$1', (int(cid),))
        await db.query('DELETE FROM questions WHERE cid=$1', (int(cid),))
        text = f'Вопрос: <b>{question[0]}</b>\n\nОтвет: <b>{answer}</b>'
